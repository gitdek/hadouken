...
my $posturl = 'https://hooks.slack.com/services/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'
    ;                                           # from Incoming Webhooks Integration

my $default_channel = '#slaughterhouse';        # override with -C flag.
...
if ( not exists $opts{t} ) {
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Downtime: $opts{a}/$opts{d}  State: Is now in a planned downtime (notifications disabled)",
            color  => '#7C19FF',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Downtime: $opts{a}/$opts{d}  State: Downtime ended by schedule (notifications enabled)",
            color  => '#7C19FF',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Downtime: $opts{a}/$opts{d}  State: Downtime has been cancelled (notifications enabled)",
            color  => '#7C19FF',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Flapping: $opts{a}/$opts{d}  State: Service detected flapping (notifications disabled)",
            color  => '#19FFF4',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Flapping: $opts{a}/$opts{d}  State: Service stopped flapping (notifications enabled)",
            color  => 'good',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{attachments} = [
        {
            fallback =>
                "Flapping: $opts{a}/$opts{d}  State: Flapping detection turned off (notifications enabled)",
            color  => '#19FFF4',
            fields => [
                {
                    title => 'Host',
    ...
    $payload->{text} =
        "$opts{t} $opts{a}/$opts{d} $opts{w} State: $opts{s} $opts{o} $opts{u} $opts{c}";
}
