...
sub new {
    ...
    $self->{def_nick_change} = $self->{nick_change} = sub {
        my ($old_nick) = @_;
        "${old_nick}_";
    };
...
sub cleanup {
    ...
    $self->{change_nick_cb_guard} = $self->reg_cb(
...
sub connect {
    ...
    my $iface    = delete $info->{iface};
    my $bindaddr = delete $info->{bindaddr};
...
sub send_long_message {
    ...
    @lines = map split_unicode_string( $encoding, $_, $line_len ), @lines;
...
sub enable_ping {
    ...
    $self->{_ping_timer} = AE::timer $int, 0, sub {
...
sub split_nick_mode {
    ...
    if ( $prefixed_nick =~ /^([\Q$pchrs\E]+)(.+)$/ ) {
        ...
        $nick = $2;
        for ( split //, $p ) {
            $mode_map{ $self->map_prefix_to_mode($_) } = 1;
        }
    }
    else {
        $nick = $prefixed_nick;
...
sub privmsg_cb {
    ...
    for (@$ctcp) {
        $self->event(
            ctcp => prefix_nick($msg),
            $msg->{params}->[0], $_->[0], $_->[1], $msg->{command}
        );
        $self->event(
            "ctcp_" . lc( $_->[0] ),
            prefix_nick($msg), $msg->{params}->[0],
            $_->[1], $msg->{command}
        );
    }
...
sub part_cb {
    ...
    $self->event(
        part => $nick,
        $chan, $self->_was_me($msg), $msg->{params}->[1]
    );
...
sub kick_cb {
    ...
    $self->event(
        kick => $kicked_nick,
        $chan, $self->_was_me($msg), $msg->{params}->[2], $kicker_nick
    );
